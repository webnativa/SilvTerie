...
define('TITULO_MODULO', 'Mergulhos');
define('_TABLE_', 'conteudo');
define('_MODULE_', 'mergulhos');
...
function save($conn) {
    ...
    if ($_POST && array_key_exists('__send_form', $_POST)) {
        ...
        if (!array_key_exists('id', $_POST)) {
            ...
            $sql = "INSERT INTO $table (titulo, sintese, texto, imagem, status, destaque, modulo, pagamento) "
                    . "VALUES "
                    . "(:titulo, :sintese, :texto, :imagem, :status, :destaque, :modulo, :pagamento)";
        }

        if (array_key_exists('id', $_POST)) {

            $sql = "UPDATE $table SET "
                    . " titulo = :titulo, sintese=:sintese, texto=:texto, imagem= :imagem, status=:status, destaque=:destaque, pagamento=:pagamento "
                    . "WHERE id = :id";
        }
